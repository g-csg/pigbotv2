...
namespace PigBot.BotCommands
{
	public class ReplaceAliReflinkCommand: IBotCommand
	{
        ...
        private static async Task<string> DeleteRef(string content)
        {
            var replaceMessage = content;

            var aliParser = new Regex(@"(http[s]?://s\.click)\S+", RegexOptions.Compiled | RegexOptions.IgnoreCase);
            foreach (Match match in aliParser.Matches(content))
            {
                try
                {
            ...
            foreach (Match match in aliMobileParser.Matches(content))
            {
		...
		public async Task Execute(SocketMessage message)
		{
			...
			var replaceMessage = await DeleteRef(message.Content);
